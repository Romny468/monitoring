...
def module_install(package):
    if os_check("") == True: os.system("python -m pip install" + package)
...
def yaml_loader(filepath):
    with open(filepath) as yml_data: data = yaml.safe_load(yml_data)
    return data
...
def main():
    ...
    send_mail(not_responsive)

# mail function
def send_mail(not_responsive):
    # open yaml file and retrieve email information
    with open("settings.yml") as yml_data:
        ...
        try:
            ...
            # create message with html to send mail
            ...
            # error codes from mail server
                ...
                return False
        except: print("error while creating email (html, message, subject, from and to.)")
